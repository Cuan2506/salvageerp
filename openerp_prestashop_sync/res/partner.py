...
class res_partner(osv.osv):
    ...
    _columns = {
                ...
                'id_default_group':fields.many2one('res.partner.category','Default Group')
                }
    
    _defaults = {
                 'is_carrier':False
                 }
    def _get_categories(self, cr, uid, shop_id, associations, context={}):
        if context.get('ext_record', '') in ['supplier','manufacturer'] or not associations:
            return [[6,0,[]]]
        if not associations.get('groups'):
            return [[6,0,[]]]
        ret_val = []
        if not isinstance(associations['groups']['group'],(list,tuple)):
            associations['groups']['group'] = [associations['groups']['group']]
        for gr_data in associations['groups']['group']:
            ext_id = self.pool.get('res.partner.category').get_int_ref(cr, uid, shop_id, gr_data['id'], context=context)
            if ext_id :
                ret_val.append(ext_id)
