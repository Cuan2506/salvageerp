...
def search_from_prestashop(self, cr, uid, shop_id, presta_ids=[], context=None):
    ...
    if res_val.get('errors'):
        if isinstance(res_val['errors']['error'],(list,tuple)):
            res_val['errors']['error'] = res_val['errors']['error'][0]
        raise osv.except_osv(
             _("Error While importing"),
            _("Reason: %s") % tools.ustr(res_val['errors']['error']['message'])
             )
    for res in res_val:
        if res_val[res] and isinstance(res_val[res], dict):
    ...
    for  presta_data,openerp_id in \
        ...
        if presta_id:
            ...
            if  not presta_data or 'errors' in presta_data:
                ...
                self.pool.get(
                    'prestashop.log').register_log(cr, uid, shop_id, self._name,
                      _("Error While writing External Record for object %s Openerp ID %s Error %s"%(self._name, openerp_id,
                                                         presta_data)), 'error',
            ...
            if 'errors' in presta_data:
                ...
                self.pool.get(
                   'prestashop.log').register_log(cr, uid, shop_id,self._name,
                     _("Error While creating External Record for object %s Openerp ID %s Error %s"%(self._name,openerp_id,
            ...
            self.pool.get(
                  'prestashop.log').register_log(cr, uid, shop_id, self._name,
                         _("Record Created for %s openerp id %s External id %s"%(self._name, openerp_id,
    ...
    for openerp_data in self.get_openerp_data(cr,uid, shop_id,
        ...
        for field in fields_translate:
            translations = self._conver_translation(cr, uid,
                                                    ...
                                                    openerp_data[field])
            default_code = self.pool.get('sale.shop').get_default_lang(cr, uid, shop_id)
            openerp_data[field] = translations[default_code]
        ...
        if openerp_id:
            ...
            try:
                ...
                self.pool.get(
                  'prestashop.log').register_log(cr, uid, shop_id, self._name,
                            _("Record Write for %s openerp id %s External id %s"%(self._name,openerp_id, ext_id)),
                ...
                self.pool.get(
                    'prestashop.log').register_log(cr, uid, shop_id, self._name,
                                    _("Error in writing to %s Values are %s error is %s"%(self._name, openerp_data,
                                                        e)), 'error', 'import',
                                                             context=context)
            ...
            try:
                ...
                self.pool.get(
                    'prestashop.log').register_log(cr, uid, shop_id, self._name,
                                 _("Error in Creating new %s Values are %s error is %s"%(self._name, openerp_data, e)),
                                             'error', 'import', context=context)
...
class preshashop_log(osv.osv):
    ...
    def register_log(self, cr, uid, shop_id, res_model, msg, state, sync_type,
                     context={}):
        logger = logging.getLogger(res_model)
        if state == 'error':
            logger.error('%s for %s >> %s'%(sync_type, shop_id, msg))
        else:
            logger.info('%s for %s >> %s'%(sync_type, shop_id, msg))
        return True
