...
class sale_shop(osv.osv):
    ...
    _columns = {
                ...
                'default_lang_id':fields.many2one('prestashop.lang','Default Lang'),
                }
    ...
    def get_presta_shop(self, cr, uid, ids, context):
        ...
        if not shop_data['verified'] :
            if not context.get('test'):
                ...
                raise osv.except_osv(_("Please Verify/Approve Connection"), '')
    ...
    def test_connection(self, cr, uid, ids, context=None):
        ...
        shop_data = self.read(cr, uid, ids, ['prestashop_config_path'], context=context)

        if shop_data[0]['prestashop_config_path'] and not os.path.exists(shop_data[0]['prestashop_config_path']):
            raise osv.except_osv(
                 _("Invalid Configuration File Path"),'')
        ...
        if not res.get('errors'):
            ...
            if isinstance(res['errors']['error'],(list,tuple)):
                res['errors']['error'] = res['errors']['error'][0]
            raise osv.except_osv(
        ...
        return connection
    def get_default_lang(self, cr, uid, shop_id, context=None):
        default_code = False
        self_obj = self.browse(cr, uid, shop_id, context)
        if not self_obj.default_lang_id:
            raise osv.except_osv(
                 _("Language Configuration"),
                _("Please Select Default Language for the Shop")
                 )
        if self_obj.default_lang_id and not self_obj.default_lang_id.lang_id:
            raise osv.except_osv(
                 _("Language Configuration"),
                _("Please Select Mapping for the Default Language")
                 )

        return self_obj.default_lang_id.lang_id.code
...
#         self.do_synch(cr, uid, ids, 'product.combination', context=new_cnt)
    ...
    def import_sale_order(self, cr, uid, ids, context={}):
        ...
        new_cnt['read_at_once'] = True
        for shop_id in ids:
            self.pool.get('delivery.carrier').import_from_prestashop(cr, uid, shop_id,
                                                               context=context)
#        self.do_synch(cr, uid, ids, 'delivery.handlings', context=new_cnt)
        self.do_synch(cr, uid, ids, 'sale.order.state', context=new_cnt)
        time.sleep(2)
        for shop_id in ids:
    ...
    def sync_all(self, cr, uid, ids, context={}):
        default_code = self.get_default_lang(cr, uid, ids[0])
