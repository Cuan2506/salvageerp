...
class sale_order_line(osv.osv):
    ...
    def _get_taxes(self, cr, uid, shop_id, associations, context={}):
        
        if not associations or not associations.get('tax',False):

            return []

        if isinstance(associations['tax'] ,(dict)):

            ext_ids = associations['tax']['id']

        else:
            ext_ids = []

            for ext_id in associations['tax']:

                ext_ids.append(ext_id['id'])

        my_list = []
        int_id = self.pool.get('account.tax').get_int_ref(cr, uid,
                              shop_id, ext_ids, context=context)
        if int_id:
...
#            if  presta_dict['unit_price_tax_incl']:
##                prod_id = self.pool.get('product.product').browse(cr, uid, self.pool.get('product.product').get_int_ref(cr, uid, shop_id, presta_dict['product_id'].get('_text',False), context=context))
##                tax_rule_id = self.pool.get('tax.rules').search(cr, uid, [('tax_rule_group_id', '=', prod_id.tax_rule_group_id.id)])
##                tax_rule_browse = self.pool.get('tax.rules').browse(cr, uid, tax_rule_id[0]).tax_id.id
#                tax_rate = (float(presta_dict['unit_price_tax_incl']) - float(presta_dict['unit_price_tax_excl']))/100
    ...
    def _set_associations(self, cr, uid, shop_id, associations, context={}):
        ...
        if associations['total_shipping'] > 0.0:
            ...
            order_shipping.update({
                                   ...
                                   'price_unit':associations['total_shipping_tax_incl'] > 0.0 and associations['total_shipping_tax_incl'] or associations['total_shipping'],
                                   'product_id':False,
                                   'product_uom_qty':1,
                                   'product_uom':1
                                   })
