...
class images_image(osv.osv):
    ...
    def import_image_prestashop(self, cr, uid, shop_id, context={}):
        if getattr(self,'_not_thread',False) and self._not_thread:
            ...
            self.import_image_prestashop_thread(cr, uid, shop_id, context,
                                                Thread=False)
    ...
    def import_image_prestashop_thread(self, cr, uid, shop_id, context={}, Thread=True):
        ...
        for self_obj in self.browse(cr, uid, ids, context=context):
            ...
            if self_obj.product_id:
                ...
                p_id = self.pool.get('product.product').get_ext_ref(cr, uid,
                                                        ...
                                                        context=context)
                i_id = self.pool.get('image.image').get_ext_ref(cr, uid,
                                                        shop_id,
                                                        self_obj.id,
                                                        context=context)
                if not p_id :
...
#                    connection.delete_images('images','categories',p_id)
            ...
            if context['image_type'] != 'products':
                ...
                if new_ids:
                    ...
                    self.set_ext_ref(cr, uid, shop_id,
                                     self_obj.id,
